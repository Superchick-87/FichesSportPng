...
<html>
     ...
     */ -->
	<body onload="javascript:affichageSchemaFoot(); menuDeroulant();">
	<header>
		<img class="logoCompetition" src="css/images/<?php echo ddc($Championnat); ?>.svg">
		<h1><?php echo $Championnat; ?><br>Présentation</h1>
		<h2>Back-office</h2>
		<hr  class="separation">
	</header>
	<h2>Rencontre</h2>
	<div id="AlertSel" class="" style="display: block;">Sélectionner vos équipes</div>
	...
	<table>
		...
		<tr>
			...
			<td><select class="SelEquipe" id="choix1" name="choix1" onchange="menuDeroulant();" >                 
            <?php
             	foreach ($grpa0 as $item) {
                    ...
                    echo'</select>';
            ?> </td>
			<td><select class="SelEquipe" id="choix2" name="choix2" onchange="menuDeroulant();">
            <?php
                 foreach ($grpa0 as $item) {
			...
			<td class="EcussonClub" align="left">
				<img onselect="menuDeroulant();" id="M2ResultatRussie" class="ImageEcussonClubDroite" style="display:none;" src=""/></td>
		</tr>
		</tbody>
	</table>
...
<!--=======================================
 ...
 ========================================-->
		...
		<div class="boiteHaut">	
				<h2>Chaînes télé</h2>
				<div id="AlertTv" class="">Choix deux chaine 2 télés max.<br/>(Facultatif)</div>
				<div class="ListeTeles">
			<?php 
				foreach ($tvs as $tv) {
	            	echo '<label class="container">
	            	<input class="single-checkbox" type="checkbox" id="sel'.$tv.'" name="tv[]" value="'.$tv.'">
	            	<img class="checkmark choix" src="css/images/tv/'.$tv.'.svg" alt="'.$tv.'">
	            	</label>';
                 }; 
            ?>
				</div>
 ...
 ========================================-->
	...
	<div class="blocHaut">
		...
		<!-- # -----------  OPTION FOOT SCHEMA  ------------->
				...
				<div id="schemaTac1" style=""><h4>Schéma tactique</h4>				
				...
				<?php
					for ($i=1; $i < $entrees; $i++) {
						...
						<div style="display:flex; justify-content: space-evenly; align-items: stretch;">
							...
							<div class="champsNumeros" style="order: 2;">
								<label for="EquipeDom'.$i.'"><h4>N°</h4></label>
								<input class="champsNumerosInp" type="number" id="EquipeDomNum'.$i.'" name="EquipeDomNum'.$i.'" style="width:50px;" placeholder="Son n°" value="'.@$_SESSION["EquipeDom".$i.""].'">
							</div>
							<div style="order: 3;">
								<label for="EquipeDomCap'.$i.'"><h4>Cap.</h4></label>
								<select id="EquipeDomCap'.$i.'" name="EquipeDomCap'.$i.'" style="width:50px; height: 30px;">';
		...
		<!-- # -----------  AFFICHAGE CHAMPS ENTRAINEURS || REMPLCANTS EQUIPE 1  ------------->
					...
					<div id="RempEquipe1">	
						...
						<label for="RemplacantsD"><h3>Remplaçants</h3></label> 
			       		<textarea required maxlength="200" id="RemplacantsD" name="RemplacantsD" cols="20" rows="40" placeholder="200 signes max."></textarea><br/>
	    			</div>
	    			<hr>
...
<!--=======================================
 ...
 =======================================-->
		...
		<!-- # -----------  OPTION FOOT SCHEMA  ------------->
			...
			<div id="schemaTac2" style=""><h4>Schéma tactique</h4>
					...
					for ($i=1; $i < $entrees; $i++) {
						...
						<div style="display:flex; justify-content: space-evenly; align-items: stretch;">
							...
							<div class="champsNumeros" style="order: 2;">
								...
								<input class="champsNumerosInp" type="number" id="EquipeExtNum'.$i.'" name="EquipeExtNum'.$i.'" style="width:50px;" placeholder="Son n°" value="'.@$_SESSION["EquipeExt".$i.""].'">
							</div>
							<div style="order: 1;">
								<label for="EquipeExtCap'.$i.'"><h4>Cap.</h4></label>
								<select id="EquipeExtCap'.$i.'" name="EquipeExtCap'.$i.'" style="width:50px; height: 30px;">';
		...
		<!-- # -----------  AFFICHAGE CHAMPS ENTRAINEURS || REMPLCANTS EQUIPE 2  ------------->
				...
				<div id="ExtRempEquipe2">	
					...
					<label for="RemplacantsE"><h3>Remplaçants</h3></label> 
		       		<textarea required maxlength="200" id="RemplacantsE" name="RemplacantsE" cols="20" rows="40" placeholder=" 200 signes max."></textarea><br/>
	    		</div>
	    		<hr>
